...
void Editor::handleEvent(Event* e) {
    if (e->getType() == MouseDownEvent::MOUSE_DOWN_EVENT) {
        ...
        for (DisplayObject* object : this->selected) {
            ...
            // Calculate grid snapping threshold (dependent on zoom)
            double threshold = Editor::GRID_SIZE * this->camera->getZoom();
            ...
            // Also, despite trunc() returning "the nearest **integer** not greater in magnitude than arg",
            // it still returns a double...hence the static_cast back to an int -.-
            object->position.x += static_cast<int>(Editor::GRID_SIZE * trunc(this->displacementX.at(object) / threshold));
            object->position.y += static_cast<int>(Editor::GRID_SIZE * trunc(this->displacementY.at(object) / threshold));
